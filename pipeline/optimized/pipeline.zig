...
inline fn fnv1a(bytes: []const u8) u64 {
...
fn runWorker(context: *Context, io: std.Io) void {
    const prefix = "task:item:";
    var payload: [24]u8 = undefined;
    @memcpy(payload[0..prefix.len], prefix);
    while (context.tasks.pop(io)) |task| {
        var number = task.id;
        var position = payload.len;
        if (number == 0) {
            position -= 1;
            payload[position] = '0';
        }
        while (number > 0) : (number /= 10) {
            position -= 1;
            payload[position] = '0' + @as(u8, @intCast(number % 10));
        }
        const digits = payload[position..];
        @memcpy(payload[prefix.len..][0..digits.len], digits);
        context.results.push(io, .{ .checksum = fnv1a(payload[0 .. prefix.len + digits.len]) });
    }
}
