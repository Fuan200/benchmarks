...
pub fn main(init: std.process.Init) !void {
    ...
    var latencies: [Tasks]u16 = undefined;
    ...
    while (context.results.pop(io)) |result| {
        latencies[received] = result.latency;
        received += 1;
    ...
    std.mem.sort(u16, &latencies, {}, std.sort.asc(u16));
    var output: [192]u8 = undefined;
    const line = try std.fmt.bufPrint(
        &output,
        "Async complete: tasks={d}, ok={d}, rate_limited={d}, errors={d}, " ++
            "latency_sum={d}, p50={d}, p95={d}, p99={d}\n",
        .{ Tasks, ok, rate_limited, errors, latency_sum, latencies[5000], latencies[9500], latencies[9900] },
    );
